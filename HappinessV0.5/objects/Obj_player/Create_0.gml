...
objectmap = layer_tilemap_get_id("Tiles_Objects");
...
function add_pickup(_pickup_to_add){
    ...
    if(_pickup_to_add == "Sword"){
        ...
        Sword_Place = _Hotbar[array_length(pickups) - 1];
        show_Sword = true;
        _Has_Sword = true;
    }

    if(_pickup_to_add == "Pills"){
        Hotbar_Pills = array_length(pickups) - 1;
        Pills_Place = _Hotbar[array_length(pickups) - 1];
        _Has_Pills = true;
    }
}
